 ...
 * @returns An object indicating success or failure, with user data or an error message.
 */
export async function getUserByEmail(email: string): Promise<{ success: true, user: User | null } | { success: false, error: string }> {
  try {
 ...
 * @returns An object indicating success or failure, with the created user data or an error message.
 */
export async function createUser(userData: Omit<User, 'id' | 'joined_date'>): Promise<{ success: true, user: User } | { success: false, error: string }> {
  try {
    const userToCreate = {
      ...userData,
